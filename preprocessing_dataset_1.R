...
data <- data %>% select("Tipo.di.viaggio", "TIME", "Value") %>%
  separate("TIME", c("Year", "Quorter"), "-")  %>%
  ...
  group_by_("Year", "Travel.Type") %>% summarise_at(vars("Value"), list(Avg = mean))

write.csv(data, "./processed_data/1.historical_data_1998_2013_type_of_journey.csv", row.names = FALSE)
